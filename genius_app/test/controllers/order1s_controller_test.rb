...
class Order1sControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create order1" do
    assert_difference('Order1.count') do
      post order1s_url, params: { order1: { comments: @order1.comments, items: @order1.items, name: @order1.name } }
    end
  ...
  test "should update order1" do
    patch order1_url(@order1), params: { order1: { comments: @order1.comments, items: @order1.items, name: @order1.name } }
